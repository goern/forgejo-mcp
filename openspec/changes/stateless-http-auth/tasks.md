...
- [ ] 2.4 **Blocker fix** — match scheme case-insensitively (RFC 7235). `bearer xyz` MUST resolve to the same token as `Bearer xyz`. Today the case-sensitive `strings.HasPrefix("Bearer ")` rejects lowercase, which then falls through to the bare-token branch (1.5), which silently downgrades to the global identity.
- [ ] 2.5 **Blocker fix** — drop the no-scheme bare-token branch (`operation/operation.go:152-154` and `:181-183`). Not in the spec. Accepting a bare token without a scheme expands the contract silently. If kept, document in proposal and README.
