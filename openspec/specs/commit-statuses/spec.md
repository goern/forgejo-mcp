...
# commit-statuses Specification

## Purpose

Paginated per-context commit statuses for a full 40-character SHA through
`get_commit_statuses`. Combined aggregate state stays on
`forgejo://repo/{owner}/{repo}/commit/{sha}/status`. A short SHA is an
error and makes no request. Commit statuses are not Actions workflow runs.
