...
# workflow-run-control Specification

## Purpose

Cancel or delete a workflow run and inspect that run's artifacts through
`cancel_workflow_run`, `delete_workflow_run`, `list_action_run_artifacts`,
and `get_action_artifact`. Cancel treats HTTP 204 as success even when the
run already finished. Delete has no preflight and surfaces a live-run
error. Artifact list is server-paged; get returns metadata only.
