...
# repo-topics Specification

## Purpose

List, replace, add, and delete a repository's topics through
`list_repo_topics`, `set_repo_topics`, `add_repo_topic`, and
`delete_repo_topic`. `set_repo_topics` is replace-all: a missing key is an
error, an empty string clears. `get_repo` joins `ListRepoTopics` so the SDK's
missing `Topics` field is not silent.
