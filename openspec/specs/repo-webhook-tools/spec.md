# Capability: repo-webhook-tools

## Purpose

MCP tools and resource templates for managing repository webhooks on a Forgejo instance. Covers the full CRUD lifecycle (list, get, create, edit, delete) plus test-delivery triggering, with two `forgejo://` resource templates for URI-addressable access. The `secret` field is never echoed in any response path.
