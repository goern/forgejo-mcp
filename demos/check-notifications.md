...
```bash
FORGEJO_URL=https://codeberg.org FORGEJO_ACCESS_TOKEN=$FORGEJO_ACCESS_TOKEN /tmp/bin84 --cli check_notifications --help 2>/dev/null
```
...
```bash
FORGEJO_URL=https://codeberg.org FORGEJO_ACCESS_TOKEN=$FORGEJO_ACCESS_TOKEN /tmp/bin84 --cli check_notifications --args '{}' 2>/dev/null | python3 /tmp/fmt_notifications.py
```
