...
| [bounded-issue-and-comment-lists.md](bounded-issue-and-comment-lists.md) | `forgejo://…/issues` and `forgejo://…/{kind}/{index}/comments` resources | Row-shaped issue lists with no bodies (measured 16.7× reduction), full-body comment threads, and the `N of M shown` truncation sentinel |
